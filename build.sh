...
# Inject favicon + Open Graph/Twitter meta tags (generated by build.php from the
# resume data) into <head>, since the jsonresume-theme-even theme doesn't add them.
metatmp="$(mktemp)"
awk -v metafile="output/meta-tags.html" '
  /<\/head>/ {
    while ((getline line < metafile) > 0) print line
  }
  { print }
' output/resume.html > "$metatmp"
mv "$metatmp" output/resume.html
echo "✓ favicon + Open Graph tags injected"
